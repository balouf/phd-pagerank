...
We should clarify that the algorithms used are, up to rewriting#footnote[@alg:pr-hybride was rewritten following the model of @alg:pr-virtuelle.], exactly those described in this work. In particular, the power methods employed are truly power methods. For a more complete study of BackRank's convergence performance, one would need to investigate how it behaves under the many convergence acceleration methods that exist @arasu01pagerank @Haveliwala99efficient @kamvar03extrapolation.
