...
#remarque[
  If the graph is not strongly connected, by @thm:reductible, convergence still occurs, but there is neither uniqueness (the dimension of the solution space equals the number of recurrent strongly connected components) nor a guarantee that the support of the solution equals $V$ (in particular if transient components exist). The existence of periodicity, on the other hand, may prevent convergence, but @thm:periodique indicates how the problem can be circumvented.
]
...
In order to replace $A$ with a stochastic matrix, and to avoid establishing an uncontrolled equivalent stochastic process through simple renormalization, a natural idea is to add transitions to dangling pages. One possible method consists in modeling the use of the _Back_ button, and will be the subject of @pr-back. Another method, proposed#footnote[Unknowingly? See @rem:bourde-bp.] initially by @Page98 (in the form of @alg:pr-completion) and studied among others by @langville04deeper, consists in defining a _default_ probability distribution $Z$ on $V$, and using it to model the behavior of the random surfer when he arrives on a dangling page. Concretely, each zero row in $A$ (which therefore corresponds to a dangling page) is replaced by the row $Z^t$.
...
#remarque[
  In @kamvar03extrapolation there is a proof of the fact that every eigenvalue other than $1$ (which is simple for $hat(A)$) is less than $d$. In @langville04deeper, it is additionally shown that the secondary eigenvalues of $hat(A)$ are equal to $d$ times those of $A$ (the multiplicities of $1$ being counted as secondary), and the authors claim that their proof is more compact than that of @kamvar03extrapolation. @thm:vp-secondaires additionally shows that the secondary eigenvectors of $hat(A)^t$ are those of $A^t$, and we claim that our proof is more compact than that of @langville04deeper. All that remains is to find a theorem more precise than @thm:vp-secondaires, with an even more compact proof...
]
...
#Thm[
  Let $A$ be a sub-stochastic matrix, $Z$ a covering distribution. If $overline(P)$ is the PageRank obtained by $mu$-compensation (cf @alg:pr-hybride), and $P$ the non-compensated PageRank, fixed point of the mapping $X -> d A^t X + (1-d) Z$, then $P$ is homogeneous to $overline(P)$.
] <thm:nonc-mu>
...
- On the other hand, if $A$, or even simply the adjacency matrix, fits in memory, the computation and incorporation of $mu$ takes a duration comparable to that of computing $A^t P$. In fact, any norm computation adds a non-negligible overhead to the computation of an iteration, and even the computation of the convergence parameter $delta$ considerably reduces performance. This leads to the SpeedRank algorithm (#ref(<alg:speedrank>, supplement: none)), which computes the non-compensated PageRank by roughly estimating the number of iterations needed for good convergence. Our experiments revealed a speed gain of more than $300%$ on iterations between SpeedRank and @alg:pr-hybride, which more than compensates for the slight overestimation of the number of iterations needed to converge.
...
The corresponding asymptotic probability vector is obtained by @alg:pr-virtuelle.
