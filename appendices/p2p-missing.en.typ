...
The inability to finish downloading files is a frequent situation in peer-to-peer file-sharing networks. While this problem sometimes seems inherently linked to the nature of the network, it appears that even _intelligent_ download networks can end up in a situation where all parts of the file are available except one.

We propose a simple and scalable file-sharing model that can apply to all block-based downloading protocols, such as BitTorrent. Simulations show that the _missing block_ case can occur even for popular files, and provide some theoretical leads.
...
#figure(
  ...
  caption: [Distribution of download progress among users in GRS],
) <download>
